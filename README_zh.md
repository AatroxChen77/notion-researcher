...
**Notion Researcher** 是一个强大的 CLI 工具，旨在简化将本地 Markdown 文档同步到 Notion 的工作流程。它填补了本地编辑与 Notion 协作工作区之间的鸿沟，通过稳健地解析 Markdown（包括表格、LaTeX 公式和图片）并将其作为原生 Notion 块发布。
...
- **🔄 灵活的同步模式**:
    - **默认追加模式**: 默认情况下，将内容追加到目标页面的底部，非常适合日常日志或连续笔记。
    - **子页面创建**: 使用 `--new` 参数强制在配置的根数据库下创建一个新的子页面。
- **🎯 精确控制**:
...
- **快速失败**: CLI 在初始化 API 客户端*之前*验证文件存在并解析 Markdown，确保对输入错误进行快速反馈。
